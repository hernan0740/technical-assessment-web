...
import { deleteAssessment, getAssessments } from '@/services/assessment.service'
...
export function AssessmentsPage() {
  ...
  useEffect(() => {
    const loadAssessments = async () => {
      try {
        ...
        setError(COPY.assessments.errors.load)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleDelete = async (assessment: Assessment) => {
    const confirmed = window.confirm(COPY.assessments.confirmDelete)
    ...
    try {
      ...
      setError(COPY.assessments.errors.delete)
    } finally {
      setDeletingId(null)
    }
  }
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-2 text-slate-600">{COPY.assessments.description}</p>
          </div>

          <Button asChild className="bg-[#0043A9] text-white hover:bg-[#00388F]">
            <Link to="/assessments/new">{COPY.assessments.createButton}</Link>
          </Button>
        </div>

        {isLoading && <p className="mt-8 text-slate-500">{COPY.assessments.loading}</p>}

        {error && <p className="mt-8 text-red-600">{error}</p>}

        {!isLoading && !error && assessments.length === 0 && (
          <div className="mt-8 rounded-xl border bg-white p-8 text-center shadow-sm">
            <h2 className="text-lg font-semibold">{COPY.assessments.empty.title}</h2>
              ...
              >
                <div className="grid gap-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                  <div className="min-w-0">
                    <h2 className="truncate text-xl font-semibold">{assessment.name}</h2>
                    ...
                    <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-500">
                      <span>
                        {assessment.timeLimitMinutes} {minuteLabel}
                      </span>

                      <span>
                        {assessment.questionCount} {questionLabel}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 sm:min-w-[270px] sm:flex-nowrap sm:justify-end">
                    ...
                    >
                      {isDeleting
                        ? COPY.assessments.deletingButton
                        : COPY.assessments.deleteButton}
                    </Button>
                  </div>
                </div>
              </article>
