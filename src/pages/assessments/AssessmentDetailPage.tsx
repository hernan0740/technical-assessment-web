...
import { deleteQuestion, getQuestionsByAssessment } from '@/services/question.service'

import { startAssessmentSession } from '@/services/assessment-session.service'
...
export function AssessmentDetailPage() {
  ...
  useEffect(() => {
    const loadAssessment = async () => {
      if (!assessmentId) {
        setError(COPY.assessmentDetail.errors.assessmentIdRequired)
  ...
  const handleStartAssessment = () => {
    ...
    navigate(`/assessments/${assessment.id}/questions/${firstQuestion.id}`)
  }

  const handleDeleteQuestion = async (question: Question) => {
    const confirmed = window.confirm(COPY.assessmentDetail.confirmDeleteQuestion)
    ...
    try {
      ...
      setError(COPY.assessmentDetail.errors.delete)
    } finally {
    ...
    return (
      <main className="px-6 py-10">
        <div className="mx-auto max-w-5xl">
          <p className="text-slate-500">{COPY.assessmentDetail.loading}</p>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="px-6 py-10">
      <div className="mx-auto max-w-5xl">
        <Link to="/assessments" className="text-sm text-slate-500 hover:text-slate-900">
        ...
        {assessment && (
          <>
            <div className="mt-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div>
                <p className="text-sm text-slate-500">{COPY.assessmentDetail.eyebrow}</p>

                <h1 className="mt-1 text-3xl font-bold tracking-tight">
                  {assessment.name}
                </h1>

                <p className="mt-2 max-w-3xl text-slate-600">{assessment.description}</p>
              ...
              {assessmentId && (
                <Button asChild variant="outline">
                  <Link to={`/assessments/${assessmentId}/questions/new`}>
              ...
              <section className="mt-8 rounded-xl border border-blue-100 bg-white p-6 shadow-sm">
                <div>
                  <h2 className="text-xl font-semibold">
                    {COPY.assessmentSession.title}
                  </h2>

                  <p className="mt-1 text-sm text-slate-600">
                    {COPY.assessmentSession.description}
                  </p>
                </div>

                <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end">
                  <div className="flex-1">
                    <label htmlFor="candidate" className="mb-2 block text-sm font-medium">
                      {COPY.assessmentSession.candidate}
                    </label>
        ...
        {error && assessment && <p className="mt-6 text-red-600">{error}</p>}

        {questions.length === 0 && (
          <div className="mt-8 rounded-xl border bg-white p-8 text-center shadow-sm">
            <h2 className="text-lg font-semibold">{COPY.assessmentDetail.empty.title}</h2>
            ...
            {assessmentId && (
              <Button asChild className="mt-5 bg-[#0043A9] text-white hover:bg-[#00388F]">
                <Link to={`/assessments/${assessmentId}/questions/new`}>
                  {COPY.assessmentDetail.createFirstQuestion}
                </Link>
              </Button>
            )}
          </div>
        ...
        {questions.length > 0 && (
          <section className="mt-8">
            <div>
              <h2 className="text-xl font-semibold">{COPY.assessmentDetail.title}</h2>
                  ...
                  >
                    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
                      <div className="min-w-0">
                        ...
                        <h3 className="mt-1 text-xl font-semibold">{question.title}</h3>
                        ...
                        <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
                          <span>
                            {COPY.assessmentDetail.labels.score}: {question.score}
                          </span>

                          <span>
                            {COPY.assessmentDetail.labels.languages}:{' '}
                            {question.allowedLanguages
                              .map((language) => COPY.languages[language])
                              .join(', ')}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex shrink-0 gap-2">
                        <Button asChild variant="outline">
                          ...
                          >
                            {COPY.assessmentDetail.editQuestion}
                          </Link>
                        </Button>
                        ...
                        >
                          {isDeleting
                            ? COPY.assessmentDetail.deletingQuestion
                            : COPY.assessmentDetail.deleteQuestion}
                        </Button>
                      </div>
                    </div>
                  </article>
